<template>
  <main class="max-w-3xl mx-auto px-4 pt-6 pb-8">
    <div class="space-y-10">
      <!--Summary-->
      <div class="space-y-8">
        <h2 class="text-3xl font-bold mb-2">About</h2>
        <p class="font-semibold leading-8">
          I was born and raised in Texas, where I currently reside with my wife Raine and 3 children (Noah, Mia, Ari).
          When I'm not building stuff, I'm busy being a dad, photography or playing golf.
        </p>
      </div>
      <!-- Goal-->
      <div class="space-y-4">
        <h4 class="uppercase text-sm font-semibold tracking-wide text-gray-400">Goal</h4>
        <p class="font-semibold leading-8 mt-6">
          Build great software and foster an environment where great software can be built. This includes focusing on a
          few ideals like thoughtful communication, encouragement, empathy, honesty, product ownership and an endless
          pursuit to learn new ideas.
        </p>
      </div>
      <!--Education-->
      <div class="space-y-4">
        <h4 class="uppercase text-sm font-semibold tracking-wide text-gray-400">Education</h4>
        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">Bachelor of Science - Electrical Engineering @ Texas A&M University</p>
          <p class="text-sm font-semibold text-gray-400">Aug 2001 - May 2006</p>
        </div>
      </div>
      <!--Experience-->
      <div class="space-y-4">
        <h4 class="uppercase text-sm font-semibold tracking-wide text-gray-400 w-1/3">Experience</h4>
        <div class="flex items-baseline justify-between">
          <p class="font-semibold leading-8">
            Vue/Rails Developer @
            <a href="https://moxiworks.com" class="text-accent-500 hover:text-accent-300 underline">MoxiWorks</a>
          </p>
          <p class="text-sm font-semibold text-gray-400">Apr 2021 - Present</p>
        </div>
        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">
            Vue/.NET Developer @
            <a href="https://asteris.com/" class="text-accent-500 hover:text-accent-300 underline">Asteris</a>
          </p>
          <p class="text-sm font-semibold text-gray-400">May 2019 - April 2021</p>
        </div>
        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">WPF/UWP Developer @ Turnkey</p>
          <p class="text-sm font-semibold text-gray-400">May 2018 - May 2019</p>
        </div>

        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">Tooling Programmer @ CNS Pantex</p>
          <p class="text-sm font-semibold text-gray-400">Jan 2017 - May 2018</p>
        </div>
        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">WPF/Firmware Developer @ Hunting Titan</p>
          <p class="text-sm font-semibold text-gray-400">Dec 2013 - Apr 2016</p>
        </div>
        <div class="flex justify-between items-baseline">
          <p class="font-semibold leading-8">Avionics Engineer @ Bell Helicopter</p>
          <p class="text-sm font-semibold text-gray-400">Sep 2008 - Dec 2013</p>
        </div>
      </div>
      <!--Projects-->
      <div class="space-y-4">
        <h4 class="uppercase text-sm font-semibold tracking-wide text-gray-400">Projects</h4>
        <p class="font-semibold leading-8">
          <a href="https://vcalendar.io" class="text-accent-500 hover:text-accent-300 underline" target="_blank"
            >V-Calendar</a
          >, an elegant calendar and datepicker plugin for Vue.
        </p>
        <p class="font-semibold leading-8">
          <a href="https://vcalendar.io" class="text-accent-500 hover:text-accent-300 underline" target="_blank"
            >GifMyCoffee</a
          >, a free coffee timer app that helps people brew various coffee recipes using gifs.
        </p>
        <p class="font-semibold leading-8">
          <a href="https://vcalendar.io" class="text-accent-500 hover:text-accent-300 underline" target="_blank"
            >Vue Screen Utils</a
          >, a dependency-free collection of utility plugins and functions for using media queries in Vue 3.
        </p>
      </div>
    </div>
  </main>
</template>

<style></style>
